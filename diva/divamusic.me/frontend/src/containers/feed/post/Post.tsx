...
import { mrUrl, coverUrl, userArUrl, origMusicUrl } from '@/utils/getS3URL';
...
const Post = ({
  ...
  const echoAudioRef2 = useRef<HTMLAudioElement>(null);
  useEffect(() => {
    if (!isPlaying) {
      mrAudioRef.current?.pause();
      arAudioRef.current?.pause();
      echoAudioRef.current?.pause();
      echoAudioRef2.current?.pause();
    }
  }, [isPlaying]);
  ...
  const handleAudioPlay = () => {
    ...
    if (echoAudioRef.current) {
      echoAudioRef.current.volume = 0.8;
      setTimeout(() => {
        echoAudioRef.current?.play();
      }, 200);
    }
    if (echoAudioRef2.current) {
      echoAudioRef2.current.volume = 0.6;
      setTimeout(() => {
        echoAudioRef.current?.play();
      }, 400);
    }
